"""
train.py — fine-tuning on TinyShakespeare with gradient accumulation + LoRA
"""
...
LORA_RANK           = 8
LORA_ALPHA          = 16.0
LORA_TARGET_MODULES = ("wq", "wv")   # add "wo", "w1" etc. for more capacity

# ── LoRA Implementation ───────────────────────────────────────────────────────
class LoRALinear(nn.Module):
    """Wraps an existing nn.Linear with a low-rank ΔW = B·A adapter.

    The base weight is frozen; only lora_A and lora_B are trained.
    ...
    def __init__(self, linear: nn.Linear, rank: int = 8, alpha: float = 16.0):
        ...
        # A: random init (breaks symmetry); B: zero init (ΔW=0 at step 0)
...
def inject_lora(
    ...
    """Walk the model and replace named Linear sub-modules with LoRALinear."""
    for module in model.modules():
        for attr_name in target_modules:
            child = getattr(module, attr_name, None)
            if isinstance(child, nn.Linear):
                setattr(module, attr_name, LoRALinear(child, rank, alpha))
    return model


def save_lora_weights(model: nn.Module, path: str = "lora_weights.pt"):
...
def load_lora_weights(model: nn.Module, path: str = "lora_weights.pt"):
...
def build_tiny_model(vocab_size: int) -> Transformer:
    ...
    model.requires_grad_(False)   # freeze all base weights
    inject_lora(model)            # add trainable LoRA adapters
    return model
...
def build_model(tokenizer: SentencePieceProcessor) -> Transformer:
    ...
    # Freeze base model, inject LoRA adapters
...
def plot_losses(steps, train_losses, val_losses):
    ...
    plt.xlabel("step")
    plt.ylabel("loss")
    plt.title("Train vs Val Loss (LoRA)")
    plt.legend()
    plt.tight_layout()

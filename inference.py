...
class LLaMA:
    ...
    model = LLaMA.build(checkpoints_dir=checkpoints_dir, tokenizer_path=f"{checkpoints_dir}/tokenizer.model", load_model=True, max_seq_len=1024, 
                        max_batch_size=len(prompts), device=device)
    print("All OK")
    print(model)
    ...
    print("\n--------------------------------\n")
    for prompt, out_token, out_text in zip(prompts, out_tokens, out_texts):
      print(f"Prompt: {prompt}")
      print(f"Tokens: {out_token}")
